...
export const useModelStore = defineStore("model", {
  ...
  actions: {
        ...
        reader.onload = async (event) => {
          try {
            ...
            if (detectedEncoding === "auto") {
              ...
              if (!detectedEncoding || detectedEncoding === "UTF-8") {
                try {
                  ...
                  // 获取UTF-8解码样本
                  const utf8Sample = new TextDecoder("UTF-8", {
                    fatal: false,
                  }).decode(uint8Array.slice(0, 1024));
            ...
            try {
              ...
              console.warn(
                `解码失败 (${detectedEncoding}), 尝试UTF-8回退:`,
                decodeError
              );
    ...
    toggleVariableSelection(variable) {
      const index = this.selectedVariables.findIndex(
        (v) => v.id === variable.id
      );
      if (index === -1) {
    ...
    transposedData() {
      ...
      //先填前5行
      let rowTemp = [];
      for (let j = 0; j < this.selectedVariables.length; j++) {
        rowTemp.push(this.selectedVariables[j].id);
      }
      dataToCsv.push(rowTemp);
      rowTemp = [];
      for (let j = 0; j < this.selectedVariables.length; j++) {
        rowTemp.push(this.selectedVariables[j].tag);
      }
      dataToCsv.push(rowTemp);
      rowTemp = [];
      for (let j = 0; j < this.selectedVariables.length; j++) {
        rowTemp.push(this.selectedVariables[j].description);
      }
      dataToCsv.push(rowTemp);
      rowTemp = [];
      for (let j = 0; j < this.selectedVariables.length; j++) {
        rowTemp.push(this.selectedVariables[j].rangeHigh);
      }
      dataToCsv.push(rowTemp);
      rowTemp = [];
      for (let j = 0; j < this.selectedVariables.length; j++) {
        rowTemp.push(this.selectedVariables[j].rangeLow);
      }
      dataToCsv.push(rowTemp);
      rowTemp = [];
      for (let j = 0; j < this.selectedVariables.length; j++) {
        rowTemp.push(this.selectedVariables[j].properties);
      }
      dataToCsv.push(rowTemp);

      for (let i = 0; i < this.selectedVariables[0].data.length; i++) {
        rowTemp = [];
        for (let j = 0; j < this.selectedVariables.length; j++) {
          rowTemp.push(this.selectedVariables[j].data[i][1]);
        }
        dataToCsv.push(rowTemp);
      }
      console.log(dataToCsv[7]);
      const csvContent = this.convertToCSV(dataToCsv);
      if (!csvContent) {
        alert("没有数据可导出");
        return;
      }
      ...
      link.setAttribute("download", `${'newfile'}.csv`);
    ...
    toggleVariableIO(variable, newIO) {
      let index1 = this.variables.findIndex((iv) => iv.id === variable.id);

      this.variables[index1].IOtype = newIO;
      console.log(this.variables[index1].IOtype);
    },
    ...
    visualizeSelectedVariables() {
      alert(
        `将可视化变量: ${this.selectedVariables.map((v) => v.tag).join(", ")}`
      );
      // 实际项目中应跳转到可视化页面
    },
    analyzeSelectedVariables() {
      alert(
        `将分析变量: ${this.selectedVariables.map((v) => v.tag).join(", ")}`
      );
      // 实际项目中应跳转到分析页面
    },
    clearAllVariables() {
